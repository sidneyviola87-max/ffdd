...
import { Home, MessageCircle, PlusCircle, User, Search, Bell, ShoppingBag, LogIn } from "lucide-react";
...
export default function SideNav() {
  ...
  const navItems = [
    ...
    { href: user ? "/profile" : "/auth", icon: User, label: user ? "Profile" : "Sign In" },
  ];

  return (
    <div className="flex flex-col h-screen sticky top-0 p-4 overflow-y-auto">
      ...
      {!user && (
        <div className="mt-auto p-3 bg-primary/5 rounded-2xl border border-primary/20">
          <p className="text-sm text-muted-foreground mb-3 leading-tight">Sign in to buy, sell, and message sellers</p>
          <Link href="/auth">
            <div className="flex items-center gap-2 bg-primary text-primary-foreground rounded-xl px-4 py-2.5 font-medium text-sm hover:bg-primary/90 transition-colors justify-center">
